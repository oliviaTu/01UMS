...
ver_conf=/opt/powercloud/ums/ums_server/version
INSTALL_DIR=/opt/powercloud/ums/ums_server
#prog_conf=/opt/wisecloud/cdn//vod/authcenter/authcenter_server/etc/authcenter.conf
...
# pid_file="/opt/wisecloud/cdn/vod/sysmanage/sysmanage_server/run/SysManage.pid"
...
process=`ps -ef |grep /opt/powercloud/ums/ums_server |grep -v "grep" |grep -v "status" |grep -v "stop" |grep -v "start"  |grep -v "restart"  |wc -l`
